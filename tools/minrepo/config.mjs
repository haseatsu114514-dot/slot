...
export const CONFIG = {
  ...
  // 分析窓（か月）: 集めたデータのうち「直近 nか月」だけを分析に使う。
  // 店の営業方針は変わるので、古いデータが今の判断を汚さないように既定は半年。
  // null にすると全期間（昔の法則の検証をしたいとき向け）。記念日分析だけは常に全期間を使う。
  analysisMonths: 6,
  ...
  // 設置が確認できないシリーズ（直近2週間のデータに出てこない）はレポートでは自動で非表示になる
  seriesWatchlist: [
    { name: "北斗", pattern: "北斗" },
